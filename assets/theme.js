...
document.addEventListener("cart:change", (event) => {
  ...
  setButtonByProductStatus(fetchCart)
});
...
var DialogElement = class extends HTMLElement {
   ...
   */
  show(animate27 = true) {
    document.querySelectorAll('x-localize')?.forEach(modal => {
...
var AccordionDisclosure = class extends CustomDetails {
  connectedCallback() {
    ...
    const resetMobileClosed = () => {
      if (this.hasAttribute('mobile-closed') && mq.matches) {
    ...
    resetMobileClosed();
    requestAnimationFrame(() => requestAnimationFrame(resetMobileClosed));

    if (mq.addEventListener) mq.addEventListener('change', resetMobileClosed);
    else mq.addListener(resetMobileClosed);
  }

  createShowAnimationControls() {
    this.style.overflow = 'hidden';

    const startH = `${this.summaryElement.clientHeight}px`;

    this.open = true;
    this.summaryElement?.setAttribute('aria-expanded', 'true');
    this.contentElement?.setAttribute('aria-hidden', 'false');

    const endH = `${this.scrollHeight}px`;

    const animationControls = timeline5([
      [this, { height: [startH, endH] }, { duration: 0.25, easing: 'ease' }],
  ...
  createHideAnimationControls() {
    ...
    const startH = `${this.clientHeight}px`;
    const endH = `${this.summaryElement.clientHeight}px`;

    const animationControls = timeline5([
      [this.contentElement, { opacity: 0 }, { duration: 0.15 }],
      [this, { height: [startH, endH] }, { duration: 0.25, at: '<', easing: 'ease' }]
    ]);
...
var PopupLocalize = class extends Modal {
  constructor() {
    super();
  }
  ...
  connectedCallback() {
    ...
    this.countries = this.dataset.countries?.split(',') || []
    if(sessionStorage.getItem('shown_localaze_poup') || this.countries.includes(Shopify.country)) return;
  ...
  show(animate27 = true){
    ...
    sessionStorage.setItem('shown_localaze_poup', 'true')
  }
};
if (!window.customElements.get("x-localize")) {
  window.customElements.define("x-localize", PopupLocalize);
}
